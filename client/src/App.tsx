...
type LobbyVoice = ReturnType<typeof useLobbyVoice>;

function VoiceStatus({ voice, variant }: { voice: LobbyVoice; variant: "lobby" | "game" }) {
...
function LobbyView({ user, lobby, connected, voice, error, setError, onLeave, onProfile }: { user: User; lobby: Lobby; connected: boolean; voice: LobbyVoice; error: string; setError: (value: string) => void; onLeave: () => Promise<boolean>; onProfile: (userId: string) => void }) {
  const { play: playAudio } = useAudio();
