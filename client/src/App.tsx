...
function ProfileDialog({ viewer, userId, onUser, onTutorial, onClose }: { viewer: User; userId: string; onUser: (user: User) => void; onTutorial: () => void; onClose: () => void }) {
  ...
  const upload = async () => {
    ...
    try {
      ...
      onUser(updated); setProfile((current) => current ? { ...current, user: updated } : current);
      setFile(null);
    } catch (reason) { setError(message(reason)); } finally { setBusy(false); }
  };
  const remove = async () => {
    setBusy(true); setError("");
    try { const updated = (await api<{ user: User }>("/profile/avatar", { method: "DELETE" })).user; onUser(updated); setProfile((current) => current ? { ...current, user: updated } : current); setFile(null); }
    catch (reason) { setError(message(reason)); } finally { setBusy(false); }
  };
  ...
  return <div className="dialog-backdrop"><section className="surface dialog profile-dialog"><div className="dialog-title"><div><p className="overline">{editable ? "Dein Konto" : "Spielerprofil"}</p><h2>Profil</h2></div><button className="button-icon" onClick={onClose} aria-label="Schließen">×</button></div><div className="profile-summary"><div className="profile-preview">{preview ? <img src={preview} alt="Neue Profilbild-Vorschau" /> : <Avatar user={displayed} large />}</div><div><strong className="profile-name">{displayed.username}</strong>{profile && <div className="stat-grid"><span><b>{profile.statistics.gamesPlayed}</b> Spiele</span><span><b>{profile.statistics.gamesWon}</b> Siege</span><span><b>{profile.statistics.podiumFinishes}</b> Podien</span><span><b>{profile.statistics.totalPenalty}</b> Strafpunkte</span><span><b>{profile.statistics.phasesLaid}</b> Phasen</span><span><b>{profile.statistics.cardsBought}</b> Käufe</span></div>}</div></div>{profile && <div className="achievement-list">{profile.achievements.map((achievement) => <article key={achievement.key}><strong>{achievement.title}</strong><span>Stufe {achievement.tier}/3 · {achievement.value}{achievement.next ? ` / ${achievement.next}` : " · vollständig"}</span><progress value={achievement.next ? achievement.value : 1} max={achievement.next ?? 1} /></article>)}</div>}{editable && <><button className="button-quiet" onClick={onTutorial}>Kurzanleitung erneut öffnen</button><label className="file-picker">JPEG, PNG oder WebP · maximal 5 MB<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /></label><div className="profile-actions"><button disabled={!file || busy} className="button-primary" onClick={() => void upload()}>Bild speichern</button><button disabled={!displayed.avatarKey || busy} className="button-danger" onClick={() => void remove()}>Bild löschen</button></div></>}{error && <p className="error" role="alert">{error}</p>}</section></div>;
}
