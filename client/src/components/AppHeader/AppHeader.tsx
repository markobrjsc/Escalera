...
export function AppHeader({ user, leftLabel, leftAudio, onLeft, onProfile, profileAudio }: { user: PublicUser; leftLabel: string; leftAudio?: string; onLeft: () => void; onProfile: () => void; profileAudio?: string }) {
  return <header className="app-header">
    <button className="logout-button" data-audio={leftAudio} aria-label={leftLabel} onClick={onLeft}>⇥</button>
    <Brand variant="compact" />
    <button className="profile-button" data-audio={profileAudio} data-tutorial-target="profile-entry" aria-label="Profil öffnen" onClick={onProfile}><Avatar user={user} /></button>
  </header>;
}
