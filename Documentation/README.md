# Escalera – Dokumentation

Dieser Ordner sammelt die fachlichen Anforderungen für die Neuimplementierung von Escalera.
Die Dokumentation wird unabhängig von einer früheren technischen Umsetzung erstellt.

## Dokumente

- [Game-Rules.md](Game-Rules.md) – verbindliche Spielregeln
- [Functional-Requirements.md](Functional-Requirements.md) – gewünschte Funktionen
- [Game-Logic.md](Game-Logic.md) – fachliche Abläufe und Zustände
- [Screens-and-UX.md](Screens-and-UX.md) – Ansichten und Bedienung
- [Responsive-Frontend.md](Responsive-Frontend.md) – Viewport-Skalierung, Mindestgrößen und Ausrichtung
- [Design-System.md](Design-System.md) – Farbpalette, Typografie und einheitliche UI-Komponenten
- [View-Specifications.md](View-Specifications.md) – verbindliche Inhalte der vier Hauptansichten
- [Data-Model.md](Data-Model.md) – benötigte fachliche Daten
- [Non-Functional-Requirements.md](Non-Functional-Requirements.md) – Qualitätsanforderungen
- [Open-Questions.md](Open-Questions.md) – noch zu treffende Entscheidungen
- [Glossary.md](Glossary.md) – einheitliche Begriffe
- [Implementation-Roadmap.md](Implementation-Roadmap.md) – grobe Reihenfolge der späteren Umsetzung
- [Target-Platform.md](Target-Platform.md) – Zielgeräte, Installation und Vollbildbetrieb
- [Technical-Architecture.md](Technical-Architecture.md) – festgelegte technische Architektur
- [Accounts-Profiles-and-Statistics.md](Accounts-Profiles-and-Statistics.md) – Konten, Profilbilder, Statistiken und Erfolge
- [Docker-and-Deployment.md](Docker-and-Deployment.md) – Container, lokale Umgebung und Bereitstellung
- [Development-Workflow.md](Development-Workflow.md) – verbindliche Ticket-, Branch- und Commit-Arbeitsweise

## Statuskennzeichnungen

- **Bestätigt:** gemeinsam festgelegt
- **Entwurf:** sinnvoller Vorschlag, noch zu prüfen
- **Offen:** Entscheidung oder Information fehlt

Die Zielplattform und technische Grundarchitektur sind seit dem 15.07.2026 festgelegt.
